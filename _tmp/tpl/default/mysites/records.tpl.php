...
// Cache Time:2017-08-18 18:07:25
			...
			<div class="pad-btm form-inline">
				<div class="row">
					<div class="col-sm-6 table-toolbar-left">
						...
						<a class="btn btn-primary" href="/mysites/controll/<?=$site_id?>">控制面板</a>
					</div>
				...
				<table class="table table-vcenter record-list">
					...
					<tbody>
						...
						<tr id="record_<?=$v['id']?>" class="<?=$v['change_status']?>" data-rcd-id="<?=$v['id']?>">
							...
							<?php if($v['dns_type']=='A' || $v['dns_type']=='CNAME'){ ?>
							<input id="<?=$v['id']?>-switch" class="toggle-switch host-<?=$v['dns_host']?>" type="checkbox" <?=ifchecked($v['cdn_status'],'on','str')?> disabled="disabled" onchange="change_status('<?=$v['dns_host']?>',this.checked)">
...
<script>
var ori = new Array();
function change_status(host,ison){
	if(ison){
		$('.host-'+host).attr('checked','checked');
	}else{
		$('.host-'+host).removeAttr('checked');
	}
}
function dns_lines(cur){
	arr = [<?php foreach($_dns_line as $v){ ?>'<?=$v?>',<?php } ?>];
	str = '';
...
function dns_types(cur){
	<?php if($site['cdn_type']=='ns'){ ?>
	arr = [<?php foreach($_dns_type as $v){ ?>'<?=$v?>',<?php } ?>];
	<?php }else{ ?>
	arr = ['A','CNAME'];
	<?php } ?>
	str = '';
...
function dns_ttls(cur){
	arr = [<?php foreach($_dns_ttl as $v){ ?>'<?=$v?>',<?php } ?>];
	str = '';
	for(i=0;i<arr.length;i++){
		if(arr[i]==cur){
			str += '<option value="'+arr[i]+'" selected="selected">'+arr[i]+'</option>';
