...
class app_core{
	...
	public static function __init_env(){
		$env_arr = array('_tmp/','sys/dblog/','_tmp/cache/','_tmp/templates/','_tmp/global/');
		for ($i=0;$i<count($env_arr);$i++){
			make_dir(APP_ROOT.$env_arr[$i]);
		}
	}
	// 开启Gzip压缩
	...
	// 初始化模板
	public static function init_tpl(){
		$file = APP_ROOT.'sys/tpl_settings.inc.php';
		...
		$user_tpl_dir = $user_tpl_dir ? "tpl/$user_tpl_dir/" : 'tpl/default/';
		$admin_tpl_dir = $admin_tpl_dir ? "tpl/$admin_tpl_dir/" : 'tpl/admin/';
		return array('user_tpl_dir'=>$user_tpl_dir,'admin_tpl_dir'=>$admin_tpl_dir);
	}
}
